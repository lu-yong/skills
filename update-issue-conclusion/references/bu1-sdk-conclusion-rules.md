...
对 `错误` issue，现有 `问题性质` 有值时，使用该值选择结论结构；字段为空时，由 update skill 按 `update-issue-conclusion/SKILL.md` 的确认协议询问用户，接受值与解析契约不在此处维护。`历史遗留` 使用普通结论结构，`当前版本修改引入` 和 `上个版本修改引入` 分别使用对应的补丁引入结构。该选择只用于本次结论草稿，不更新 `问题性质` 字段。
