...
3. **Load the conclusion rules.** Read the local reference [references/bu1-sdk-conclusion-rules.md](references/bu1-sdk-conclusion-rules.md) first. It is a focused cache of the “处理任务要求” rules relevant to conclusions and classifications; record that the draft used the local reference and its wiki version. Refresh the authenticated Redmine wiki page [Redmine使用规则](https://git.nationalchip.com/redmine/projects/bu1-sdk/wiki/Redmine%E4%BD%BF%E7%94%A8%E8%A7%84%E5%88%99#%E5%A4%84%E7%90%86%E4%BB%BB%E5%8A%A1%E8%A6%81) only when the reference is missing, more than 30 days old, the user requests a refresh or reports a rule change, or the issue needs an uncovered special rule. When refreshing, update the local reference's wiki version and relevant content. If refresh fails but the local reference exists, continue with the cache and disclose that fact; if both refresh and the local reference are unavailable, ask the user for the applicable rules and do not invent a format.
...
2. Before writing, re-read the issue and verify that the project is still `bu1-sdk`, the tracker has not changed, all required field IDs are unchanged, and current values have not changed in a way that invalidates the draft. If any of these changed, refresh the analysis and ask for confirmation again.
