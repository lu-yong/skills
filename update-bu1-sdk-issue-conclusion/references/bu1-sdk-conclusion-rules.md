...
对 `错误` issue，现有 `问题性质` 有值时，使用该值选择结论结构；字段为空时，由 update skill 按自身确认协议展示 `1`/`2`/`3` 选择提示并严格解析；提示模板与解析契约在 `update-bu1-sdk-issue-conclusion/SKILL.md`，不在此处重复维护。接受值为单个 `1`、`2` 或 `3`，其它输入重新展示同一组选项；`1` 使用普通结论结构，`2` 和 `3` 分别使用对应的补丁引入结构。该选择只用于本次结论草稿，不更新 `问题性质` 字段。
